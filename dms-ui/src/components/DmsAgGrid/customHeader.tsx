...
const CustomHeader: React.FC<CustomHeaderProps> = (props) => {
    ...
    useEffect(() => {
        if (props) {
            let filterDataL = [
                ...new Set(
                    props?.dataList?.map(
                        (item: any) => item[props?.column?.colId]
                    )
                ),
            ];
    ...
    const filtrationMethod = () => {
        const filterDataList: any = props?.dataList?.filter(item => treeDataCheckout.includes(item[props?.column?.colId]));
    ...
    const content = (
        <div className={styles.sorterPopover}>
            <div className={styles.sorterPopover_Top}>
                <span onClick={(event) => { onSortRequested('desc', event), setOpen(false) }}>
                    <span><Icon icon="local:sortDes" height="15" width="15" /></span>
                    {intl.formatMessage({ id: 'dms.common.operate.desOrder' })}
                </span>
                <span onClick={(event) => { onSortRequested('asc', event), setOpen(false) }}>
                    <span><Icon icon="local:sortAsc" height="15" width="15" /></span>
                    {intl.formatMessage({ id: 'dms.common.operate.ascOrder' })}
                </span>
            </div>
            <div className={styles.sorterPopover_Bottom}>
                <Input
                    className={styles.sorterPopover_Bottom_input}
                    onChange={onChange}
                />
                <Space size="small" style={{ marginBottom: 3 }}>
                    ...
                    >
                        {intl.formatMessage({ id: 'dms.common.operate.allsSelect' })}
                    </Button>{' '}
                    <Button
                        size="small"
                        onClick={() => {
                    ...
                    >
                        {intl.formatMessage({ id: 'dms.common.operate.cancel.allsSelect' })}
                    </Button>
                </Space>
                <Tree
                    treeData={treeData}
    ...
    return (
        <div className={styles.headerIcon}>
            ...
            {props.displayName && (
                <Popover content={content} placement="bottomLeft" trigger="click" open={open}
                    ...
                    {!descSort && !ascSort && (
                        <Icon icon="local:collect" height="13" width="13" />
                    )}
                </Popover>
            )}
        </div>
